...
cd servers/filesystem-sandbox     && npm install && npm test    # 50 sandbox + tool + config + public-surface + atomic-write tests
cd servers/github-gists           && npm install && npm test    # 59 config + client (redaction + rate-limit diag + cfg validation + filename-trim) + error-message-format + tool + public-surface tests
