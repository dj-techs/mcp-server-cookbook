...
**Duration:** ~30 min. **Issue:** [#22](https://github.com/dj-techs/mcp-server-cookbook/issues/22). **PR:** [#23](https://github.com/dj-techs/mcp-server-cookbook/pull/23).
...
## 2026-05-22 — Issue #16: capture script under tools/ drives three load-bearing security surfaces

**Duration:** ~30 min. **Issue:** [#16](https://github.com/dj-techs/mcp-server-cookbook/issues/16). **PR:** [#24](https://github.com/dj-techs/mcp-server-cookbook/pull/24).

The cookbook's "Demo" issue asked for a 60-second walkthrough as a GIF or video AND a capture script that makes the demo deterministically re-runnable. The GIF/video itself needs a human operator with a screen recorder (Claude Desktop or Claude Code CLI driving the MCP integration), so that part stays JT's recording step; this session shipped the script, the smoke test, the CI wiring, and the README rewrite that point at all of it.

`tools/capture_demo.sh` runs three surfaces in sequence with no DB, no network, and no real `GITHUB_TOKEN`. Each one exercises the load-bearing primitive of one shipped server — the part of the threat model the demo is actually proving, not just any happy-path tool call. **Surface 1** drives `postgres-readonly`'s `guardQuery()` (the layer that refuses an attempted write before the DB role's read-only check even runs, per D-004) on three allowed reads and four rejected statements (INSERT, DROP, multi-statement, UPDATE with a comment-hidden keyword), printing the named reasons inline. **Surface 2** does real disk IO under a `tempfile.TemporaryDirectory`, creating a real symlink whose target is outside the allow-list so `Sandbox.resolve()` actually exercises `os.path.realpath`'s symlink-follow against D-006; runs seven cases (inside / traversal / absolute-outside / symlink-outside / relative / null-byte / control-char). **Surface 3** injects a fake `fetch` into `GistsClient` — one path returns a fixture gist with a 150KB file so `projectGist`'s per-file truncation cap fires (and a 12-byte file stays untouched), then a 401 path where the configured bearer is a recognizable token sentinel literal. The script prints `GithubApiError.message + status + endpoint + reason` and asserts the sentinel is absent from both the message and the `JSON.stringify`d error — that's D-007's contract. The fake fetch also verifies the sentinel IS on the `Authorization` header before responding, which pins the other half of D-007 ("on the wire, never in error context").

Lock-against-bitrot: `tools/check-capture-demo.{mjs,test.mjs}` is a parallel to the repo's existing `check-readme`, `check-spec-version`, and `check-architecture-doc` checkers, but structured around a subprocess invocation. The pure helpers — `REQUIRED_PATTERNS`, `FORBIDDEN_LITERALS`, `assertCaptureOutput`, `runCaptureDemo` — are exported so the 9 `node:test` unit tests can drive the assertion logic against synthetic stdout (drop one required pattern → fires "missing-required" with the dropped pattern's name; append the token sentinel → fires "forbidden-present" with D-007 named in the error). One of the unit tests is a cross-file invariant: it reads `tools/capture_demo.sh` as text and asserts the script's `TOKEN_SENTINEL` literal matches the asserter's constant, so a future edit that changes the sentinel in one file but not the other can't silently make the redaction check a no-op. A new `capture-demo` CI job installs Node 20, Python 3.11, `npm ci`s in `servers/postgres-readonly` and `servers/github-gists` for the `tsx` binaries, `pip install -e`s `servers/filesystem-sandbox-py`, then runs the checker followed by its own unit tests.

The README's Demo section now enumerates the three surfaces with their D-NNN annotations and the exact quickstart command sequence; the "GIF capture pending #16" honest framing stays, because the GIF still doesn't exist. The PR is **draft** until JT records the GIF and ticks the first two acceptance bullets; the third (capture script under `tools/`) is now satisfied.

This is the fourth capture script in the portfolio pattern (after `llm-eval-harness`, `llm-cost-optimizer`, `prompt-regression-suite`, and `rag-production-kit` all landed similar scripts last week) — and the first one in this repo, so `tools/check-capture-demo.{mjs,test.mjs}` extends the established `tools/check-*` shape rather than introducing a new structural pattern. The cookbook's per-server independence (D-002) made this the most complex orchestration of the four — three separate runtimes (Node + Python) and three separate node_modules / venv prerequisites — but the result still runs hermetically in a single `bash` invocation.

**Open questions / blockers:** None. The GIF/video itself is the recording step (JT, ~30 min with the local tooling per the issue's "Steps" section).

**Next session:** Multi-issue day loop — re-run repo selection on the remaining 11 repos.
